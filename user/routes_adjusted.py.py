...
#         @wraps(fn)
#         def wrapper(*a, **k):
#             if not _is_admin() and "admin" in roles:
#                 return jsonify({"error":"Unauthorized","required_roles":roles}), 403
#             return fn(*a, **k)
#         return wrapper
#     return deco
...
#     keep = {
#         "unit_price","annualized_pct_premium","non_annualized_pct_premium",
#         "currency","option_type","value_date","strike","derived_strike","forward_rate"
#     }
...
#     rows = fetch_admin_premia_rows(pair, option_type, transaction_type, valuation_date)  # expects [{maturity_date, premium_pct, strike?}]
#     pts = []
#     for r in rows:
#         T_i = (parse_date(r["maturity_date"]) - valuation_date).days/365.0
#         if T_i <= 0: continue
#         rd_i, rf_i = get_rates_at_T(pair, valuation_date, T_i)
#         K_i = float(r["strike"]) if r.get("strike") is not None else strike_from_spot(S, rd_i, rf_i, T_i)
#         annualized_i = float(r["premium_pct"]) / T_i     # parity: non-annualized → annualized
#         sigma_i = implied_volatility(option_type.lower(), S=S, K=K_i, T=T_i, rd=rd_i, rf=rf_i, annualized_premium=annualized_i)
#         if math.isfinite(sigma_i):
#             pts.append((T_i, sigma_i))
#     if not pts:
#         raise ValueError("No solvable IV points")
#     # linear interpolate on T
#     pts.sort()
#     Ts, Sig = zip(*pts)
#     if T_star <= Ts[0]: sigma_star = Sig[0]
#     elif T_star >= Ts[-1]: sigma_star = Sig[-1]
#     else:
#         import bisect
#         j = bisect.bisect_left(Ts, T_star); T0,T1 = Ts[j-1],Ts[j]; S0,S1 = Sig[j-1],Sig[j]
...
#         # σ: users cannot set it; admins may override; otherwise derive from admin premia → σᵢ → interpolate σ(T)
#         sigma = None
...
#         if sigma is None:
#             try:
#                 currency_pair = f"{currency.upper()}/TND"
#                 sigma, vol_points = _build_sigma_from_admin_premia(
#                     pair=currency_pair,
#                     option_type=option_type,
#                     transaction_type=(data.get("transaction_type") or "buy").lower(),
#                     S=spot_rate, K_star=final_strike, T_star=T,
#                     valuation_date=datetime.today().date()
#                 )
#             except Exception as e:
#                 return jsonify({"message": f"Unable to derive implied volatility: {e}", "debug": debug_logs}), 422
#         else:
#             vol_points = None
...
#     # ---- sigma: users cannot set it; admins may override; else derive via admin premia engine ----
#     sigma = None
#     if _is_admin() and data.get("volatility_input") is not None:
#         try:
#             sigma = float(data["volatility_input"])
...
#     if sigma is None:
#         try:
#             currency_pair = f"{currency}/TND"
#             sigma, vol_points = _build_sigma_from_admin_premia(
#                 pair=currency_pair,
#                 option_type=option_type,
#                 transaction_type=transaction_type,
#                 S=spot_rate, K_star=strike_value, T_star=T,
#                 valuation_date=today
#             )
#         except Exception as e:
#             return jsonify({"message": f"Unable to derive implied volatility: {e}"}), 422
#     else:
#         vol_points = None  # no points if admin forced sigma
...
#             # Per-row strike: IRP unless you later support per-row admin strikes
#             K_i = strike_from_spot(spot, rd_i, rf_i, T_i)
